...
pub async fn serve_pac(ConnectInfo(_addr): ConnectInfo<std::net::SocketAddr>) -> String {
    // let proxy_url = format!("PROXY {}:8080; DIRECT", addr.ip());
    let proxy_url = format!("PROXY prxy.villebiz.com; DIRECT");
    format!(
        r#"
        function FindProxyForURL(url, host) {{
            ...
            // Use the proxy server's address as the proxy
            var proxy = "{proxy_url}";
            return proxy;
        }}
        "#
